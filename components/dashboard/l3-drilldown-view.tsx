// components/dashboard/l3-drilldown-view.tsx
'use client';
...
interface L3DrilldownViewProps {
  fiscalYear?: number;
}

export function L3DrilldownView({ fiscalYear = 2569 }: L3DrilldownViewProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['l3-drilldown', parentId, selectedYear],
    queryFn: async () => {
  ...
  const handleDrillDown = (categoryId: string) => {
    router.push(`/dashboard/l4?categoryId=${categoryId}&fiscalYear=${selectedYear}`);
  };

  const handleBack = () => {
    router.push('/dashboard/l2?parentId=' + searchParams.get('parentId') + '&type=' + type);
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">
              {type === 'revenue' ? 'รายได้' : 'รายจ่าย'} - ระดับ L3
            </h1>
            <p className="text-muted-foreground">
              รายละเอียดหมวดหมู่ระดับที่ 3
            </p>
          </div>
        </div>
